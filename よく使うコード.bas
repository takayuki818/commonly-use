...
Sub 処理時間計測()
    ...
    MsgBox "処理が完了しました" & vbCrLf & vbCrLf & "処理時間：" & Now - 始時
    Unload 実行中
End Sub
Sub 並替基本形とカナ削除()
    Dim 最下行, 最右列 As Long
...
Sub 条件付書式等設定例()
    ...
    End With
End Sub
Sub リストによる入力規則設定(リスト)
'256字以上の文字列形式でリスト形式の入力規則設定するとファイルが破損することに注意！
'→　本例ではリスト設定セル参照形式にすることでファイル破損を回避しています
    Dim 範囲式 As String
    With Sheets("プルダウン設定")
        .Cells(1, 1).Resize(Rows.Count, 1).ClearContents
        .Cells(1, 1).Resize(UBound(リスト), 1) = リスト
        範囲式 = "=プルダウン設定!$A$1:$A$" & UBound(リスト)
    End With
    With Sheets("入力フォーム")
